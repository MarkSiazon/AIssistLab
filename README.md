...
`npm run smoke:production` expects `npm run build` to have completed, starts `next start` against the demo workspace, verifies local-device APIs return the intended production guard response, and checks the built Settings, Skills, Chat, Export, Editor, and Guided Builder pages render nonblank desktop/mobile views without horizontal overflow, broken landmarks/headings/ARIA references, missing accessible control names, undersized action targets, or unexpected browser errors.
