...
The helper may print `blocked` readiness when the local provider is intentionally incomplete, such as Claude CLI mode with local CLI calls disabled or API mode without an API key. Treat that as a sanitized setup snapshot, not as a failed automated smoke run. Finish the account-backed chat check only after you intentionally configure a provider you are allowed to use.
