...
const source = readFileSync("scripts/smoke/runners/local.mjs", "utf8");
const interactionSource = readFileSync(
  "scripts/smoke/assertions/interaction.mjs",
  "utf8",
);
...
const selectedDownloadCall = source.slice(
  source.indexOf("const download = await downloadByButton"),
  source.indexOf("const downloadPath = path.join"),
);
assert.equal(
  selectedDownloadCall.includes(
    String.raw`downloadByButton(page, "Download Selected \\+ Diagnostics", {`,
  ) && selectedDownloadCall.includes("exact: false"),
  true,
