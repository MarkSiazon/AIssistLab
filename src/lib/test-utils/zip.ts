...
export function createZip(entries: Record<string, string>): Buffer {
...
export function extractZipEntries(zip: Buffer): Record<string, string> {
  ...
  if (endOffset === -1) throw new Error("ZIP end record not found");
...
export function spoofLocalUncompressedSize(zip: Buffer, size: number): Buffer {
  const next = Buffer.from(zip);
  next.writeUInt32LE(size, 22);
  return next;
}
